...
import com.mlkit.sampletext.util.FileUtils;
import com.mlkit.sampletext.util.PCMToWav;

import java.io.IOException;
import java.util.HashMap;
import java.util.Map;

public class TtsAudioActivity extends BaseActivity implements View.OnClickListener, SeekBar.OnSeekBarChangeListener {

    public TtsAudioActivity() {
        super(true);
    }
    ...
    public void onClick(View v) {
        ...
        if (vId == R.id.close) {
            editText.setText("");
        } else if (vId == R.id.btn_play) {// Use the system player to play the cached audio.
            playAudio(AUDIO_FILE_NAME_WAV);
        } else if (vId == R.id.btn_add) {
            if (mlTtsEngine == null) {
                return;
            }
            if (isPause) {
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        int id = seekBar.getId();
        if (id == R.id.volumeSeek) {  // volumeSeek
            volumeVal = seekBar.getProgress() / 10f;
            updateConfig();
        } else if (id == R.id.speedSeek) {  // speedSeek
